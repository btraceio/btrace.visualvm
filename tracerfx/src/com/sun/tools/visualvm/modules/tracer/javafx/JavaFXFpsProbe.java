 ...
 */
class JavaFXFpsProbe extends JavaFXProbe {

    private static final String NAME = "Average FPS";
    private static final String DESCR = "Monitors average frame per second rate";
    ...
    long[] getValues(List<Attribute> metrics) {
        return new long[] {
            Math.max(((Double)metrics.get(0).getValue()).longValue(), 0)
        };
    }
    ...
    private static final ProbeItemDescriptor[] createItemDescriptors() {
        return new ProbeItemDescriptor[] {
            ProbeItemDescriptor.lineItem("Average FPS", "Monitors average frame per second rate", ItemValueFormatter.DEFAULT_DECIMAL, 0, ProbeItemDescriptor.MAX_VALUE_UNDEFINED)
        };
    }

    private static final String[] createAttrs() {
        return new String[] { "averageFPS" }; // NOI18N
    }
